...
# schedule meeting
def schedule_meeting(user_id, adoption_id, date, time, notes):
    conn = sqlite3.connect("adoptions.db")
    c = conn.cursor()
    c.execute("""
        INSERT INTO meetings (user_id, adoption_id, date, time, notes) VALUES (?, ?, ?, ?, ?)
    """, (user_id, adoption_id, date, time, notes))
    conn.commit()
    conn.close()
